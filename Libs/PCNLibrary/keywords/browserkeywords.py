...
class BrowserKeywords(LibraryComponent):
    ...
    def open_my_browser(self, setup):
        ...
        if setup.get('browser').lower() == 'firefox':
            options = FirefoxOptions()
            if setup.get('headless').lower() == 'true':
        ...
        elif setup.get('browser').lower() == 'chrome':
            ...
            if setup.get('headless') == 'True':
                self.enable_download_in_headless_chrome(driver,
                                                        curr_path + self.format_os_path(self.CHROME_DOWNLOAD_LOCATION))
        else:
            if self.get_current_os().lower() == 'windows':
                driver = webdriver.Edge(executable_path=curr_path +
                                                            self.format_os_path(self.WINDOWS_EDGE_DRIVER_PATH))
            else:
                raise AssertionError("Edge browser does not support for: %s " % self.get_current_os().lower(), "Just for windows")
        driver.get(setup.get(BuiltIn().get_variable_value("${PORTAL_URL}")).get('url'))
